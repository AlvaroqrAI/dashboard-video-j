...
const sectionLabel: React.CSSProperties = { fontSize:10, fontWeight:600, color:'#4A4960', textTransform:'uppercase' as const, letterSpacing:'0.1em', marginBottom:8 }
const iconBtn: React.CSSProperties = { width:28, height:28, borderRadius:8, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', display:'inline-flex', alignItems:'center', justifyContent:'center', color:'#8B8A99', cursor:'pointer', flexShrink:0 }
...
export default function Calls() {
  ...
  const selectedIndex = selected ? rows.findIndex(r => r.call_id === selected.call_id) : -1
      ...
      {selected && (
        <div onClick={() => setSelected(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            style={{ width:'100%', maxWidth:640, maxHeight:'88vh', overflowY:'auto', background:'#181922', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16 }}>

            {/* Header */}
            <div style={{ display:'flex', alignItems:'center', gap:12, padding:'18px 22px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ width:40, height:40, borderRadius:'50%', background:'#1E1F2B', color:'#9B8FEF', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:700, flexShrink:0 }}>
                {(apptInfo[selected.call_id]?.name || selected.from_number || '?').charAt(0).toUpperCase()}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:14.5, fontWeight:700, color:'#F1F0F5' }}>{apptInfo[selected.call_id]?.name || selected.from_number || 'Llamada'}</div>
                <div style={{ fontSize:11, color:'#8B8A99', marginTop:2, fontFamily:'monospace' }}>{selected.from_number || '—'} · {fmtDate(selected.start_timestamp)}</div>
              </div>
            ...
            <div style={{ padding:'20px 22px', display:'flex', flexDirection:'column', gap:18 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
                <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                  {selected.call_successful != null && (
                    <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:11.5, fontWeight:500, color: selected.call_successful ? '#34D399' : '#F87171', border:'1px solid rgba(255,255,255,0.08)', borderRadius:7, padding:'5px 10px' }}>
                      <span style={{ width:5, height:5, borderRadius:'50%', background: selected.call_successful ? '#34D399' : '#F87171', display:'inline-block' }}/>
                      {selected.call_successful ? 'Efectiva' : 'Fallida'}
                    </span>
                  )}
                  <span style={{ fontSize:11.5, fontWeight:500, color:'#8B8A99', border:'1px solid rgba(255,255,255,0.08)', borderRadius:7, padding:'5px 10px' }}>
                    {selected.agent_name || selected.agent_id || 'Sin agente'} · {selected.direction === 'inbound' ? 'Entrante' : 'Saliente'}
                  </span>
                  {selected.user_sentiment && (
                    <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:11.5, fontWeight:500, color: sentimentColor(selected.user_sentiment), border:'1px solid rgba(255,255,255,0.08)', borderRadius:7, padding:'5px 10px' }}>
                      <span style={{ width:5, height:5, borderRadius:'50%', background: sentimentColor(selected.user_sentiment), display:'inline-block' }}/>
                      {selected.user_sentiment}
                    </span>
                  )}
                </div>
                <span style={{ fontSize:13, fontWeight:700, color: durationColor(selected.duration_ms) }}>{fmtDuration(selected.duration_ms)}</span>
              </div>

              {selected.call_summary && (
                <div>
                  <div style={sectionLabel}>Resumen</div>
                  <p style={{ fontSize:12, color:'#C4BCFF', lineHeight:1.6, margin:0 }}>{selected.call_summary}</p>
                </div>
              )}

              <div style={{ background:'#131318', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:16 }}>
                <div style={sectionLabel}>Reproductor</div>
              ...
              <div>
                <div style={sectionLabel}>Transcripción de la llamada</div>
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', padding:'14px 22px', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
              <button type="button" onClick={() => setSelected(null)}
                style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, padding:'8px 18px', fontSize:12, fontWeight:600, color:'#F1F0F5', cursor:'pointer', fontFamily:'inherit' }}>
                Cerrar
              </button>
            </div>
          </div>
